...
class particle:
    ...
    def train_particle(self):
        ...
        for index, layer in enumerate(step_weight):
            ...
            new_v = new_v + self.parm["local_acc"] * local_rand * (
                self.local_best_model.get_weights()[index] - layer
            )
            new_v = new_v + self.parm["global_acc"] * global_rand * (
                self.global_best_model.get_weights()[index] - layer
            )
...
if __name__ == "__main__":
    ...
    for i in range(NUMOFCLIENTS):
        pso_model.append(
            particle(
                particle_num=i,
                client=init_model(train_data_shape=x_train.shape[1:]),
                x_train=client_data[i][0],
                y_train=client_data[i][1],
            )
        )
    ...
    for epoch in range(EPOCHS):
        ...
        server_evaluate_acc.append(
            server_model.evaluate(x_test, y_test, batch_size=BATCH_SIZE, verbose=1)
        )
